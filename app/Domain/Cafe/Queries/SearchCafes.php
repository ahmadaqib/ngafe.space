    ...
    public function run(?string $q, array $categorySlugs, ?float $lat, ?float $lng, ?string $area, string $city = 'makassar'): Collection
    {
        ...
        if (filled($q)) {
            if ($query->getConnection()->getDriverName() === 'pgsql') {
                $query->whereRaw('name % ?', [$q])->orderByRaw('similarity(name, ?) DESC', [$q]);
            } else {
                $query->where('name', 'like', '%'.str_replace(' ', '%', trim($q)).'%');
            }
        }
        if ($lat !== null && $lng !== null) {
            ...
            $query->orderByDesc('quality_score')->orderByDesc('rating_count');
        }
