...
final class OpeningHours
{
    ...
    {
        $override = collect($cafe->opening_hours_override ?? [])->first(fn ($o) => $now->toDateString() >= $o['date_start'] && $now->toDateString() <= $o['date_end']);
        ...
        return new OpeningStatus($open, $open ? 'Buka sekarang' : 'Tutup', $override['label'] ?? null);
    }
}
