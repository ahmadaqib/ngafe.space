...
### Task 5.4: Security headers + CSP + rate limiter global (§10 hardening lapis 3) ✅ (selesai 2026-07-19)

**Files:** `app/Http/Middleware/SecurityHeaders.php`, `bootstrap/app.php`, `config/rate_limits.php`, `app/Providers/AppServiceProvider.php` · Test: `tests/Feature/SecurityHeadersTest.php`, `tests/Feature/Auth/LoginRateLimitTest.php`

- [x] CSP: `default-src 'self'`; `img-src` self + domain R2 (dibaca dari `filesystems.disks.r2.url`, jadi otomatis ikut kalau env berubah) + `data:`; `script-src 'self' 'nonce-{random per request}'`. Nonce didaftarkan via `Vite::useCspNonce()` di awal middleware — `@vite`, `@livewireScripts`, `@livewireStyles`, dan dua `<script>` inline (registrasi SW, IntersectionObserver sticky-CTA) otomatis ikut nonce yang sama. `X-Frame-Options: DENY`, `X-Content-Type-Options: nosniff`, `Referrer-Policy: strict-origin-when-cross-origin`, `Permissions-Policy: geolocation=(self), camera=(), microphone=(), payment=()`.
  - **Temuan empiris via `tests/Browser/*` (bukan asumsi):** tanpa `'unsafe-eval'`, Alpine bawaan Livewire gagal total (EvalError di `new Function(...)` yang dipakai buat evaluasi `x-data`/directive) — sheet login, form 3 langkah, dan tombol share semua rusak. `'unsafe-eval'` dipertahankan di `script-src` dengan komentar yang menjelaskan kenapa (Alpine build CSP-safe/precompiled tidak dipakai). `style-src 'self' 'unsafe-inline'` juga dipertahankan (dibutuhkan `@livewireStyles`); ini pelonggaran yang jauh lebih rendah risiko dibanding `script-src` sehingga tidak diperjuangkan lebih jauh.
- [x] Named rate limiters terpusat di `config/rate_limits.php` (review 3/jam·10/hari, foto 20/hari, report 10/hari, content-appeal 3/hari + verify 5/jam) — `ReviewGuards`, `AttachPhotos`, `SubmitReport`, `SubmitContentAppeal` sekarang baca dari config, bukan angka hardcode; enforcement tetap di Action (bukan middleware) karena butuh exception domain + copy ramah, sesuai §9. Login per IP (satu-satunya yang cocok dengan `throttle:` middleware biasa) didaftarkan sebagai named limiter `RateLimiter::for('login', ...)` di `AppServiceProvider`, dipasang ke `/auth/google/redirect` dan `/auth/google/callback`. Usul cafe (F8) belum ada limiter — menyusul Phase 6 saat Action-nya dibangun, bukan diprediksi sekarang.
- **Keamanan yang ketemu saat kerja (bukan bagian task ini, tapi ditemukan & langsung ditutup):** blok JSON-LD (Task 5.1) pakai `JSON_UNESCAPED_SLASHES`, yang mematikan escaping `/`→`\/` bawaan `json_encode` — nama/alamat cafe yang mengandung `</script>` bisa memutus tag script dan menyuntik markup. Diperbaiki + ada regression test yang memuat payload asli dan dikonfirmasi gagal tanpa fix, lulus dengan fix (lihat commit). Commit.
