...
class CafeController extends Controller { public function show(string $city,string $slug){ $cafe=Cafe::query()->where(['city'=>$city,'slug'=>$slug,'status'=>CafeStatus::Active])->with(['categories','photos','reviews'=>fn($q)=>$q->where('status','published')->with('photos')])->firstOrFail(); return response()->view('cafe.show',['cafe'=>$cafe,'opening'=>OpeningHours::statusNow($cafe,CarbonImmutable::now())])->header('Cache-Control','public, max-age=300'); } }
